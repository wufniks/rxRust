/// `impl_local_share_both` is a macro helper to implement both the
...
/// ```
#[macro_export]
macro_rules! impl_local_share_both {
  (
...
(
  ...
  (
    ...
    impl_local_share_both!(@$ctx, $($before)*);
  };
  (
    @impl_local,
  ...
  // enter replace
  ($($t:tt)*) => {
    impl_local_share_both!(@replace, impl_local, [] $($t)*);
    impl_local_share_both!(@replace, impl_shared, [] $($t)*);
  };
}
pub mod impl_local {
  use crate::prelude::*;
